...
api_key = 'AIzaSyCl6dUqUISXSHDX4sgyjzSeQ6oCBw50UPw'
    ...
    try:
        ...
        async with stdio_client(server_params) as (read, write):
            ...
            async with ClientSession(read, write) as session:
                ...
                # Debug: Print all available tools and specifically check for open_paint
                print("\nAvailable Tools:")
                open_paint_tool = None
                for tool in tools:
                    print(f"- {tool.name}: {getattr(tool, 'description', 'No description')}")
                    print(f"  Input Schema: {tool.inputSchema}")
                    if tool.name == "open_paint":
                        open_paint_tool = tool
                        print("\nFound open_paint tool!")
                        print(f"Description: {getattr(tool, 'description', 'No description')}")
                        print(f"Input Schema: {tool.inputSchema}")
                
                if not open_paint_tool:
                    print("\nWARNING: open_paint tool not found in available tools!")
                ...
                while iteration < max_iterations:
                    print('*****************************', iteration)
                    if iteration == 5:
                        current_query =  "add a text of final answer"
                    ...
                    elif response_text.startswith("FUNCTION_CALL_PAINT:"):
                        _, function_info = response_text.split(":", 1)
                        parts = [p.strip() for p in function_info.split("|")]
                        func_name, params = parts[0], parts[1:]
                        
                        print(f"\nDEBUG: Processing paint action: {func_name}")
                        if func_name == "open_paint":
                            print("DEBUG: Special handling for open_paint tool")
                            print("DEBUG: Checking if open_paint tool exists...")
                            tool = next((t for t in tools if t.name == "open_paint"), None)
                            if not tool:
                                print("ERROR: open_paint tool not found!")
                                print("Available tools:", [t.name for t in tools])
                                raise ValueError("open_paint tool not found in available tools")
                            
                            print("DEBUG: Found open_paint tool, calling it...")
                            try:
                                result = await session.call_tool("open_paint", arguments={})
                                print("DEBUG: open_paint tool called successfully")
                                print(f"DEBUG: Result: {result}")
                                
                                iteration_response.append(
                                    f"In the {iteration + 1} iteration you opened the paint application."
                                )
                                last_response = result
                                continue
                            except Exception as e:
                                print(f"ERROR: Failed to call open_paint tool: {str(e)}")
                                raise
                        else:
                            try:
                                # Find the matching tool to get its input schema
                                ...
                                if not tool:
                                    print(f"DEBUG: Available tools: {[t.name for t in tools]}")
                                    raise ValueError(f"Unknown paint tool: {func_name}")

                                print(f"DEBUG: Found paint tool: {tool.name}")
                                print(f"DEBUG: Paint tool schema: {tool.inputSchema}")

                                # Prepare arguments according to the tool's input schema
                                arguments = {}
                                schema_properties = tool.inputSchema.get('properties', {})
                                print(f"DEBUG: Paint tool schema properties: {schema_properties}")

                                for param_name, param_info in schema_properties.items():
                                    if not params:  # Check if we have enough parameters
                                        raise ValueError(f"Not enough parameters provided for paint tool {func_name}")
                                        
                                    value = params.pop(0)  # Get and remove the first parameter
                                    param_type = param_info.get('type', 'string')
                                    
                                    print(f"DEBUG: Converting paint parameter {param_name} with value {value} to type {param_type}")
                                ...
                                print(f"DEBUG: Final paint arguments: {arguments}")
                                print(f"DEBUG: Calling paint tool {func_name}")

                                result = await session.call_tool(func_name, arguments=arguments)
                                print(f"DEBUG: Paint tool result: {result}")
                                
                                # Add the paint action to iteration response
                                iteration_response.append(
                                    f"In the {iteration + 1} iteration you called paint tool {func_name} with {arguments} parameters."
                                )
                                last_response = result
                                
                            except Exception as e:
                                print(f"DEBUG: Error in paint tool execution: {str(e)}")
                                print(f"DEBUG: Error type: {type(e)}")
                                import traceback
                                traceback.print_exc()
                                iteration_response.append(f"Error in paint tool execution: {str(e)}")
                                break
                        iteration += 1
